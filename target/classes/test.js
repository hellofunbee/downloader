var Base64 = {
    _keyStr: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=", encode: function (j) {
        var m = "";
        var d, b, g, p, c, l, k;
        var h = 0;
...
var shortUrlArr = ["url", "t", "dwz", "suo"], hostMap = {
...
var app = new Vue({
    ...
    data: {
        link: "",
        linkFromInit: false,
        submitBtnClass: {disabled: false},
        errorTip: "",
        isMP4File: false,
        requestSuccess: false,
        showAllSupportLink: false,
        showClearBtn: false,
        requestResult: {video: "", text: "", cover: ""}
    },
    watch: {
        link: function (b, a) {
            if (b.length > 0) {
    ...
    methods: {
        gaga: function (a, b) {
            ...
            var b = this, c = Math.random().toString(10).substring(2), a = this.generateStr(this.link + "@" + c).toString(10);
            $.ajax({
                type: "POST",
                url: "http://service.iiilab.com/url/unshort",
                xhrFields: {withCredentials: true},
                crossDomain: true,
                data: {link: b.link, r: c, s: a},
                dataType: "json",
                success: function (d) {
                    if (d.succ) {
                        b.link = d.data;
...
/**
 * Created by weifengxu on 2018/4/4.
 */
